...
class GPT:
    ...
    "You are tasked with correcting the output of google tesseract. The \
    ...
    def __init__(self, filename, model = "gpt-3.5-turbo", temperature = 0.1):
        # Load textfile
    ...
    def build(self):
        log.info(f"Building ThreadPool")
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(self.chunks)) as executor:
            chunk_futures = {
    ...
    def write(self):
        self._concatenate()
        with open(f"{self.filename.split('.')[0]}-IMPR.txt", "w+", encoding="utf-8") as f:
            f.write(self.output)
    ...
    def _req(self, chunk):
        ...
        log.info(f"{response.usage.total_tokens = }")
        GPT.total_tokens += response.usage.total_tokens
        log.info(f"{response.choices[0].finish_reason = }")
